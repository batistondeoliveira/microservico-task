...
public class TasksControllerIntegrationTest {
	...
	@Test
	public void givenUrl_whenSuccessOnGetsResponseAndJsonHasOneTaks_thenCorrect() {
		get("/api/todo/task/1").then().statusCode(200)
			.assertThat().body("name", equalTo("name1"), "description", equalTo("Primeira tarefa"));
	}
	
	@Test
	public void givenUrl_whenSuccessOnGetsResponseAndJsonHasTwoTask_thenCorrect() {
		get("/api/todo/task/2").then().statusCode(200)
			.assertThat().body("name", equalTo("name2"), "description", equalTo("Segunda tarefa"));
	}
}
